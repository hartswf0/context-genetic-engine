...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  ...
  switch (type) {
    ...
    case 'SAVE_CONFIG': {
      ...
      chrome.storage.local.set({ genoma_v14_model_config: config }, () => {
...
async function openChildArtifactPage(payload = {}) {
  const pack = { child: payload.child || {}, state: payload.state || {}, openedAt: Date.now(), version: 'v14' };
  await chrome.storage.local.set({ genoma_v14_program_artifact: pack });
...
async function loadConfig(incoming = {}) {
  const stored = await chrome.storage.local.get(['genoma_v14_model_config']);
  return normalizeConfig({ ...(stored.genoma_v14_model_config || {}), ...incoming });
}
...
async function encodePromptGenomeModel(payload) {
  ...
  requireModel(config);
  const system = [
    'You are GENOMA Prompt Genome Compiler v14, an LLM-first prompt-genome encoder and program-text compiler for filmable UI artifacts.',
    ...
    'Do not dump raw DOM. Compress into six prompt codons and eight or fewer operation systems. More with less.',
    'Prompt codons must be explicit enough to regenerate a steerable UI idea better than the page code. Treat the prompt genome as critical infrastructure.',
...
async function expressChildModel(payload) {
  ...
  requireModel(config);
  const system = [
    'You are GENOMA Program Text Compiler v14.',
    'The prompt genome is the main infrastructure. Your job is to compile it into runnable program text: a standalone HTML/CSS interface artifact that looks strong on camera.',
    ...
    'childUI.html must be substantial but not cluttered: one strong hero/control surface, 3-6 preserved content stations, navigation/link systems, media/reference zones, and clear calls to action using original URLs when available. Prioritize a filmable composition over exhaustive listing.',
    'childUI.css must be static CSS scoped to the artifact. Avoid imports.',
    'The artifact must visibly embody the directive and prompt genome. It should feel like a designed program, not a generic summary. Use bold spatial hierarchy, visible states, and operational controls. Avoid raw JSON, giant walls of text, or dense debug UI.',
...
function showGenomaPageStatus(payload = {}) {
  const state = String(payload.state || payload.title || 'GENOMA').slice(0, 80);
  const event = String(payload.event || payload.message || '').slice(0, 220);
  const mode = String(payload.mode || payload.tone || 'idle');
  ...
  if (!hud) {
    ...
    Object.assign(hud.style, {
      ...
      boxShadow:'5px 5px 0 #000', maxWidth:'410px', lineHeight:'1.25', pointerEvents:'none'
    });
    document.documentElement.appendChild(hud);
  }
  hud.style.background = mode === 'busy' || mode === 'info' ? '#d7f1ff' : mode === 'error' ? '#ffd8d8' : mode === 'done' ? '#d9ffe7' : '#fff';
  hud.innerHTML = `<style>@keyframes gdom{0%{transform:translateX(0)}100%{transform:translateX(265px)}}@keyframes gblink{50%{opacity:.25}}</style><div>GENOMA · ${escapeHTML(state)}</div><div style="font-weight:700;margin-top:4px">${escapeHTML(event)}</div><div style="margin-top:6px;border:2px solid #000;height:18px;background:#fff;position:relative;overflow:hidden"><b style="position:absolute;left:4px;top:2px;width:12px;height:12px;border:2px solid #000;background:#ff8c00;animation:gdom 2s linear infinite"></b><span style="position:absolute;right:5px;top:1px;font-size:10px;animation:gblink 1s steps(2,end) infinite">F0→DNA→CODE</span></div>`;
  clearTimeout(window.__genomaStatusTimer);
  if (mode !== 'busy' && !/CHILD|VIEW|ERROR|COMPILE|GENOME|ARTIFACT/i.test(state)) window.__genomaStatusTimer = setTimeout(() => hud.remove(), 4500);
