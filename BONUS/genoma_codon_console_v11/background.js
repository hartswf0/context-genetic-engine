...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  ...
  switch (type) {
    ...
    case 'TRANSCRIBE_PAGE_MODEL': {
      transcribePageWithModel(payload).then(out => sendResponse({ ok: true, ...out })).catch(e => sendResponse({ error: e.message }));
      return true;
    }
    case 'CHAT_CODON_MODEL': {
      chatOperatingCodon(payload).then(out => sendResponse({ ok: true, ...out })).catch(e => sendResponse({ error: e.message }));
      return true;
    }
    case 'PLAN_PATCH_MODEL': {
      planPatchWithModel(payload).then(out => sendResponse({ ok: true, ...out })).catch(e => sendResponse({ error: e.message }));
      return true;
    }
...
async function transcribePageWithModel(payload) {
  const config = await loadConfig(payload.config || {});
  requireModel(config);
  const system = [
    'You are GENOMA Codon Console, an LLM-first webpage geneticist.',
    'You do not dump DOM nodes. You compress a live webpage evidence packet into a small operating map.',
    'Return JSON only.',
    'Required shape: {"programTheory":{...},"operatingCodons":[...]}',
    'Create 8 to 12 operating codons maximum. Each codon is a semantic control unit, not a raw DOM node.',
    'Codon fields: id,type,label,locus,controls,payload,anchors,sourceEvidence,allowedMutations,forbiddenMutations,state,dominance,patchPolicy.',
    'Use only selectors from the provided evidence anchors or "body". Group many anchors into one codon when they form one system.',
    'Prefer codons like GLOBAL_STYLE, NAVIGATION_SYSTEM, PROJECT_CARD_SYSTEM, IMAGE_SYSTEM, BIO_TEXT, CONTACT_FORM, WAYFINDING, INTERACTION_AFFORDANCE.',
    'Every codon must answer what it controls and what must not break.',
    'No generic remake language. No full-page replacement. No raw application code.'
  ].join('\n');

  const evidence = payload.genome || {};
  const prompt = JSON.stringify({
    directive: payload.directive || '',
    kernel: String(payload.kernel || '').slice(0, 8000),
    evidence,
    allowedSelectorRule: 'Use only evidence.anchors[*].selector or body.',
    requiredOutput: {
      programTheory: { purpose: 'what this live page does', invariants: ['what must not break'], leveragePoints: ['semantic systems worth mutating'] },
      operatingCodons: [
        {
          id: 'navigation_system', type: 'HPL', label: 'Navigation System', locus: 'wayfinding',
          controls: 'all internal links and page jumps', payload: 'selector-bound navigation system',
          anchors: ['provided selectors only'], sourceEvidence: { text: 'brief evidence summary' },
          allowedMutations: ['outline', 'hover affordance', 'labels'],
          forbiddenMutations: ['change href', 'remove link', 'replace container'], state: 'EXON', dominance: 0.9, patchPolicy: 'preserve function'
        }
      ]
    }
  }, null, 2);
  ...
  if (!parsed || !Array.isArray(parsed.operatingCodons)) throw new Error('Model did not return { operatingCodons: [...] }. Try a reasoning model or simpler directive.');
  const selectors = new Set(['body', ...((evidence.anchors || []).map(a => a.selector).filter(Boolean))]);
  parsed.operatingCodons = parsed.operatingCodons.slice(0, 14).map((c, i) => normalizeOperatingCodon(c, i, selectors, evidence));
  return { raw, transcription: parsed };
}

async function chatOperatingCodon(payload) {
  const config = await loadConfig(payload.config || {});
  requireModel(config);
  const system = [
    'You are GENOMA Bound Codon Mutator.',
    'Operate only on the bound codon. Do not change id unless missing. Do not invent selectors.',
    'Return JSON only: the full updated codon object.',
    'The mutation must update the visible codon payload/controls/allowedMutations/patchPolicy, not just explain.',
    'Preserve anchors exactly unless removing invalid duplicate anchors.'
  ].join('\n');
  const prompt = JSON.stringify({ instruction: payload.message || '', kernel: String(payload.kernel || '').slice(0, 8000), boundCodon: payload.codon || {} }, null, 2);
  const raw = await callModel({ system, prompt, config });
  ...
  codon.anchors = payload.codon?.anchors || codon.anchors || [];
  return { raw, codon: { ...payload.codon, ...codon } };
}

async function planPatchWithModel(payload) {
  const config = await loadConfig(payload.config || {});
  requireModel(config);
  const system = [
    'You are GENOMA Actual Page Patch Compiler.',
    'You patch a live webpage using selector-bound operations. You do not generate a new webpage.',
    'Return JSON only: {"report":"...","operations":[...]}',
    'Allowed operations: setStyle, setAttribute, insertHTML, setText.',
    'Never use scripts, inline handlers, external resources, javascript: URLs, remove, hide, replaceHTML, or destructive operations.',
    'Never setText on body, main, nav, header, footer, section, article, form, or any container with interactive descendants.',
    'Preserve links, buttons, forms, inputs, href, src, action, method, type, name, value, and purchase/navigation paths.',
    'Prefer styling existing anchors, adding small reversible labels/HUDs, aria/title hints, and non-destructive overlays.',
    'Use only selectors available inside the provided codons anchors or body.'
  ].join('\n');
  const prompt = JSON.stringify({
    mode: payload.mode || 'patch', directive: payload.directive || '', kernel: String(payload.kernel || '').slice(0, 8000),
    boundCodon: payload.codon || null, activeCodons: payload.codons || [], evidenceStats: payload.genome?.stats || {},
    requiredOutput: { report: 'short user-facing patch summary', operations: [ { op: 'setStyle', selector: 'provided selector only', styles: { outline: '3px solid #000' }, reason: 'why this expresses the codon' } ] }
  }, null, 2);
  const raw = await callModel({ system, prompt, config });
  const parsed = parseJSONLike(raw);
  if (!parsed) throw new Error('Model did not return a patch JSON object.');
  parsed.operations = sanitizeOperationListV11(parsed.operations || [], payload.codons || [], payload.codon || null).slice(0, 80);
  return { raw, patch: parsed };
}

function requireModel(config) {
  if (!config?.model || !config?.endpoint) throw new Error('Configure provider/model/endpoint first.');
  if (config.provider !== 'local' && !config.apiKey) throw new Error(`${config.provider} API key required. Open MODEL and paste a key.`);
}

function normalizeOperatingCodon(c, i, allowedSelectors, evidence) {
  let anchors = Array.isArray(c.anchors) ? c.anchors : (c.selector ? [c.selector] : []);
  anchors = anchors.filter(s => allowedSelectors.has(s)).slice(0, 50);
  if (!anchors.length) anchors = (evidence.anchors || []).slice(i * 4, i * 4 + 4).map(a => a.selector).filter(Boolean);
  return {
    id: String(c.id || `codon_${i}`).replace(/[^a-zA-Z0-9_-]/g, '_').slice(0, 80),
    type: String(c.type || 'TXT').toUpperCase().slice(0, 8),
    label: String(c.label || c.name || `Operating Codon ${i + 1}`).slice(0, 100),
    locus: String(c.locus || 'site').slice(0, 80),
    controls: String(c.controls || c.payload || '').slice(0, 900),
    payload: String(c.payload || c.controls || '').slice(0, 1500),
    anchors,
    sourceEvidence: c.sourceEvidence || {},
    allowedMutations: listify(c.allowedMutations || ['style', 'labels', 'affordance']).slice(0, 10),
    forbiddenMutations: listify(c.forbiddenMutations || ['delete content', 'break functionality', 'replace containers']).slice(0, 10),
    state: c.state === 'INTRON' ? 'INTRON' : 'EXON',
    dominance: Number(c.dominance ?? 0.5),
    patchPolicy: String(c.patchPolicy || 'safe selector-bound patch').slice(0, 300)
  };
}

function listify(x) { return Array.isArray(x) ? x.map(v => String(v).slice(0, 120)) : [String(x).slice(0, 120)]; }

function sanitizeOperationListV11(ops, codons, focusCodon) {
  const allowedSelectors = new Set(['body']);
  const add = c => (c?.anchors || []).forEach(s => allowedSelectors.add(s));
  (codons || []).forEach(add); add(focusCodon);
...
function annotateCodonsOnPage(codons, states, boundId) {
  ...
  for (const c of active) {
    const selectors = Array.isArray(c.anchors) && c.anchors.length ? c.anchors : [c.selector].filter(Boolean);
    for (const sel of selectors.slice(0, 40)) {
      ...
      label.textContent = `${c.type || 'CODON'} · ${c.label || c.locus || ''}`.slice(0, 50);
