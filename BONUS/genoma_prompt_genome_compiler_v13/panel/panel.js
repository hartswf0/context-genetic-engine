...
async function init(){
  collectEls();
  bindEvents();
  els.kernel.value = localStorage.getItem('genoma_v13_kernel') || DEFAULT_KERNEL;
  await loadConfig();
  setStage('IDLE', 0, 4, 'Encode the page into a prompt genome. Then compile the prompt into program text.', 'idle');
  renderAll();
  log('STATUS','Loaded. Final-shot mode: prompt genome first, program text second. No page takeover.');
}
...
function renderModelLine(extra=''){
  ...
  els.modelText.textContent = missing ? `${c.provider}: key needed` : `R:${c.reasoningModel||'unset'} · F:${c.fastModel||'unset'}${extra?` · ${extra}`:''}`;
  return !missing;
}
...
async function saveConfig(){
  config = formConfig();
  localStorage.setItem('genoma_v13_kernel', els.kernel.value);
  const res = await send('SAVE_CONFIG', {config});
  if(!res.ok) return fail(res.error || 'Config save failed');
  renderModelLine('saved');
  setStage('MODEL READY',0,4,`Saved ${config.provider}. Reasoning=${config.reasoningModel}; Fast=${config.fastModel}.`,'done');
...
async function testReasoningModel(){
  try{
    ...
    setStage('MODEL READY',0,4,res.text||'Model OK','done');
...
async function encodePromptGenome(){
  try{
    config=formConfig(); ensureModel(config);
    setStage('CAPTURING F0 EVIDENCE',1,4,'Reading live page content, anchors, links, media, forms, and visible structure…','busy');
    await pageHUD('CAPTURING F0 EVIDENCE','Reading actual page evidence.');
    ...
    setStage('ENCODING PROMPT GENOME',2,4,'Reasoning model is turning page evidence into prompt infrastructure…','busy');
    await pageHUD('ENCODING PROMPT GENOME','The model is extracting the reusable prompt genome.');
    ...
    setStage('PROMPT GENOME READY',2,4,`Prompt genome ${promptGenome.length} codons. Operation systems ${operationGenome.length}. Now compile program text.`,'done');
    await pageHUD('PROMPT GENOME READY',`${promptGenome.length} codons encoded. Press COMPILE PROGRAM TEXT.`);
...
async function compileProgramText(){
  try{
    ...
    setStage('COMPILING PROGRAM TEXT',3,4,'Reasoning model is using the prompt genome to write the standalone UI artifact…','busy');
    await pageHUD('COMPILING PROGRAM TEXT','Prompt genome is producing program text/code.');
    ...
    setStage('PROGRAM TEXT READY',4,4,`Artifact compiled: ${programArtifact.title}. Open artifact page to see it full-size.`,'done');
    await pageHUD('PROGRAM TEXT READY','Open the artifact page. The live source page was not overwritten.');
...
async function openArtifact(){
  ...
  setStage('ARTIFACT OPENED',4,4,'Standalone program text artifact opened in a new tab.','done');
...
function exportPayload(){ return {version:'v13',directive:els.directive.value,generation,site:siteGenome,promptGenome,operationGenome,fitnessBaseline,programArtifact,childFitness,lastRaw}; }
...
function setStage(state, step, total, event, mode='idle'){
  ...
  if(mode==='busy') timer=setInterval(()=>{ const s=Math.floor((Date.now()-startedAt)/1000); els.elapsedText.textContent=`${String(Math.floor(s/60)).padStart(2,'0')}:${String(s%60).padStart(2,'0')}`; },250);
  else els.elapsedText.textContent='00:00';
  renderModelLine();
}
async function pageHUD(title,message){ await send('SHOW_PAGE_STATUS',{title,message,tone:'info'}).catch(()=>{}); }
