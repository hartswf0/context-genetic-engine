...
function collectEls() {
  for (const id of [
    ...
    'boundMeta','boundCard','codonChat','btnMutateCodon','btnSelectNext','childMeta','childSummary','btnReinstall','btnClearChild','btnExport',
...
async function loadConfig() {
  ...
  els.reasoningModel.value = config.reasoningModel || config.model || 'gpt-4o-mini';
  els.fastModel.value = config.fastModel || 'gpt-4o-mini';
...
function applyProviderDefaults() {
  const d = {
    ...
    openai: ['https://api.openai.com/v1/chat/completions','gpt-4o-mini','gpt-4o-mini'],
...
async function expressChildUI() {
  try {
    ...
    setStage('CHILD UI ACTIVE', 5, 5, 'Child phenotype is visible on the page. Toggle ORIGINAL / CHILD / DIFF.', 'done');
    log('RECV', `Child installed. ${childPhenotype.title || 'Untitled phenotype'}`);
  } catch(e) { fail(e.message || String(e)); }
}
