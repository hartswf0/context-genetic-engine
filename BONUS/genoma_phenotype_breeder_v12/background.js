...
const PROVIDER_DEFAULTS = {
  ...
  openai: { endpoint: 'https://api.openai.com/v1/chat/completions', reasoningModel: 'gpt-4o-mini', fastModel: 'gpt-4o-mini' },
...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  ...
  switch (type) {
    ...
    case 'SAVE_CONFIG': {
      ...
      chrome.storage.local.set({ genoma_v12_model_config: config }, () => {
...
async function loadConfig(incoming = {}) {
  const stored = await chrome.storage.local.get(['genoma_v12_model_config']);
  return normalizeConfig({ ...(stored.genoma_v12_model_config || {}), ...incoming });
}
...
async function encodePromptGenomeModel(payload) {
  ...
  requireModel(config);
  const system = [
    'You are GENOMA Phenotype Breeder, an LLM-first prompt-genome encoder.',
...
async function expressChildModel(payload) {
  ...
  requireModel(config);
  const system = [
    'You are GENOMA Child Phenotype Expressor.',
    'Do NOT patch the live DOM. Generate a child UI phenotype as a static overlay/sibling layer that can be toggled against the F0 page.',
    'The prompt genome is the main infrastructure. Use active EXON codons to express the child.',
    'Use the current page as the default fitness baseline. Preserve content, routes, interaction identity, and page-specific meaning.',
    ...
    'childUI.css must be static CSS scoped to the child layer. Avoid imports. Links in child HTML may use original URLs from evidence only.',
    'The child should feel like a real steerable UI idea, not a generic webpage.'
  ].join('\n');
  const prompt = JSON.stringify({
    ...
    requiredShape: {
      childUI: { title:'Child phenotype title', summary:'what changed and why', html:'<main>static child UI</main>', css:'.child{...}' },
...
function captureF0Page(url, title) {
  ...
  return { schemaVersion:'genoma.phenotype.v12', specimen:{url,title,capturedAt:Date.now(),viewport:{w:innerWidth,h:innerHeight},userAgent:navigator.userAgent}, stats:{anchors:anchors.length,links:anchors.filter(a=>a.tag==='a').length,buttons:anchors.filter(a=>a.tag==='button').length,forms:anchors.filter(a=>a.tag==='form').length,inputs:anchors.filter(a=>['input','textarea','select'].includes(a.tag)).length,images:anchors.filter(a=>a.tag==='img').length}, programTheorySeed:{purpose:`F0 page organism: ${title || location.hostname}`, invariants:['Use F0 as fitness baseline.','Preserve content, links, forms, navigation, images, and identity.','Child UI is a reversible sibling phenotype, not a destructive patch.','Prompt genome is the main artifact.']}, visibleText, anchors };
...
function installChildPhenotype(child, mode) {
  ...
  Object.assign(host.style, { position:'fixed', inset:'0', zIndex:'2147483000', display:'none', pointerEvents:'auto' });
  ...
  shadow.innerHTML = `<style>
    ...
    .frame{min-height:100vh;background:#fff;color:#000;overflow:auto;border-left:0 solid #000;box-sizing:border-box}
    .genoma-bar{position:sticky;top:0;z-index:5;display:flex;justify-content:space-between;align-items:center;gap:8px;border-bottom:4px solid #000;background:#fff;color:#000;padding:8px 10px;font:900 12px ui-monospace,monospace;letter-spacing:.08em;text-transform:uppercase;box-shadow:0 4px 0 #000}
    ...
    .child{padding:14px;box-sizing:border-box}
    a{color:#000;text-decoration:underline;text-decoration-thickness:2px}button,a{cursor:pointer}
    ${safe.css}
  </style><div class="frame"><div class="genoma-bar"><span>GENOMA CHILD PHENOTYPE</span><span>${escapeHTML(safe.title)}</span></div><div class="diff-panel"><b>DIFF MODE:</b> F0 remains underneath/alongside. This child is a reversible sibling layer. ${escapeHTML(safe.diffSummary)}</div><div class="child">${safe.html}</div></div>`;
  document.documentElement.appendChild(host);
  window.__genomaChildMode = mode || 'child';
  setChildMode(window.__genomaChildMode);
  return { installed:true, mode: window.__genomaChildMode };
  ...
  function sanitizeChild(c){
    ...
    return { title:String(c.title || 'Child UI').slice(0,120), summary:String(c.summary || '').slice(0,1000), diffSummary:String(c.diffSummary || c.diff?.summary || '').slice(0,1000), html:t.innerHTML, css:String(c.css || '').replace(/@import[^;]+;/g,'').replace(/url\([^)]*\)/g,'none').slice(0,20000) };
  }
...
function setChildMode(mode) {
  ...
  if (mode === 'child') { Object.assign(host.style, { left:'0', right:'0', top:'0', bottom:'0', width:'auto', height:'auto', pointerEvents:'auto' }); }
  if (mode === 'diff') { Object.assign(host.style, { left:'50vw', right:'0', top:'0', bottom:'0', width:'50vw', height:'auto', pointerEvents:'auto' }); }
  if (host.shadowRoot) {
  ...
  return { mode, visible: mode !== 'original' };
}
